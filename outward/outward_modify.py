...
class OutwardModifyFrame(tk.Frame):
    def __init__(self, parent, theme, get_colors, *args, **kwargs):
        ...
        self.modify_frame.pack(side="top", fill="x", pady=5)
    ...
    def build_title1(self):
        colors = self.get_colors()
        self.title_label1 = tk.Label(self, text="Sales - Modify Options", font=("Arial", 22, "bold"),
    ...
    def on_modify_option_change(self, *args):
        if self.modify_option.get() == "Customer":
            ...
            if self.current_name and self.current_phone:
                ...
                products = customer_backend.get_products_by_name_phone_and_date(self.current_name, self.current_phone, '', '')
    ...
    def show_purchaser_details(self, name, phone, place):
        ...
        if self.remaining < 0:
            ...
            to_pay_text = f"To Pay: {refund_due:.2f}"
        else:
            to_pay_text = f"To Get: {self.remaining:.2f}"
        ...
        options = [("Product", "Product"), ("Transactions", "Transactions"), ("Customer", "Customer")]
    ...
    def show_themed_confirmation(self, name, place, phone):
        colors = self.get_colors()
        msg = f"Customer details updated to:\n\nName: {name}\nPlace: {place}\nPhone: {phone}"
    ...
    def on_update_details(self):
        ...
        if not new_phone or not new_place:
            ...
            return
        other = customer_backend.get_customer_by_phone(new_phone)
        if other and (other[0] != name or other[2] != self.current_place):
            ...
            return
        updated = customer_backend.update_customer_phone_place(
            old_name=name,
        ...
        if updated:
            ...
            self.show_themed_error("Update Failed", "Could not update customer details.")
    ...
    def reset_date_filter(self):
        ...
        products = customer_backend.get_products_by_name_phone_and_date(
    ...
    def show_edit_form(self, product):
        ...
        self.edit_frame = tk.Frame(self.modify_frame, bg=colors["bg"], bd=2, relief="groove", width=400)
        self.edit_frame.grid(row=0, column=1, sticky="ns", padx=5, pady=5)
        ...
        tk.Label(self.edit_frame, text="Edit Product", font=("Arial", 13, "bold"),
                bg=colors["bg"], fg=colors["fg"]).pack(pady=(5, 10))
        tk.Label(
    self.edit_frame,
    ...
    fg="#cc6600"  # Orange/brown for 'note'
       ...
       # Quantity (readonly)
        ...
        tk.Entry(self.edit_frame, textvariable=self.edit_desc_var, bg=colors["entry_bg"], fg=colors["fg"]).pack(fill="x", padx=5, pady=2)

        # Amount (readonly)
        tk.Label(self.edit_frame, text="Amount:", bg=colors["bg"], fg=colors["fg"]).pack(anchor="w", padx=10)
        tk.Label(self.edit_frame, text=str(amount), bg=colors["entry_bg"], fg=colors["fg"], anchor="w").pack(fill="x", padx=5, pady=2)
    ...
    def update_product(self, old_product):
        ...
        prod_id = customer_backend.get_product_id_by_details(
        ...
        success = customer_backend.update_product_by_id(prod_id, new_item, new_desc)
        if success:
            products = customer_backend.get_products_by_name_phone_and_date(
    ...
    def show_return_form(self, product):
        ...
        def process_return():
            ...
            # --- Find product row id ---
            product_id = customer_backend.get_product_id_by_details(
                self.current_name, self.current_phone, item, qty, price, description, amount, date
            )
            if not product_id:
            ...
            customer_backend.update_product_quantity_and_amount_by_id(product_id, new_qty, new_amount)

            # --- Update purchaser in DB ---
            purchases = customer_backend.get_customer_by_name_phone(self.current_name, self.current_phone)
            if purchases:
                ...
                customer_backend.update_customer_amounts(purchaser_id, new_total, new_remaining)
                new_paid = customer_backend.get_amount_paid_by_customer_id(purchaser_id)

                # Decide status based on new totals
                if new_total == new_paid and new_remaining == 0:
                    customer_backend.update_customer_status(purchaser_id, "Completed")
                else:
                    customer_backend.update_customer_status(purchaser_id, "Pending")



            # Confirmation
            self.show_themed_message(
                f"Returned {return_qty} units of '{item}'.\nPaid Amount: {refund_amount:.2f}\n",
    ...
    def _delete_product_confirmed(self, product, confirm_win):
        ...
        if not product_id:
            ...
            return
        purchases = customer_backend.get_customer_by_name_phone(self.current_name, self.current_phone)
        customer_backend.delete_product_by_id(product_id)
    ...
    def show_product_deleted_confirmation(self, item, qty, amount):
        colors = self.get_colors()
        msg = f"Product '{item}' ({qty} units) deleted.\nAmount {amount:.2f} removed from Customer total."
        win = tk.Toplevel(self)
    ...
    def build_transaction_filter_fields(self):
        ...
        transactions = customer_backend.get_transactions_by_name_phone_and_date(self.current_name, self.current_phone, '', '')
    ...
    def on_transaction_filter(self):
        ...
        transactions = customer_backend.get_transactions_by_name_phone_and_date(name, phone, start, end)
        self.show_transaction_cards(transactions)
    ...
    def build_refund_form(self, parent):
        ...
        tk.Label(parent, text="Mark Paid Amount", font=("Arial", 14, "bold"), bg=colors["bg"], fg=colors["fg"]).pack(pady=(20, 10))
        ...
        def on_refund():
            ...
            purchaser = customer_backend.get_refund_customer_by_name_phone(self.current_name, self.current_phone)
            if not purchaser:
            ...
            remarks = f"DEBITED {refund_amt:.2f}"

            success = customer_backend.add_customer_payment(
                purchaser_id, payment_id, now, refund_amt, transaction_type='debit', remarks=remarks
            )
            if success:
                self.show_themed_message("Refunded", f"Debited {refund_amt:.2f} is noted.")
