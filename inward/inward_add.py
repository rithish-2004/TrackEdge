...
class InwardAddFrame(tk.Frame):
    ...
    def build_title(self):
        ...
        self.title_label = tk.Label(self, text="Add Stock", font=("Arial", 22, "bold"),
    ...
    def on_name_type(self, event):
        ...
        results = db_backend.search_purchasers_by_name(prefix)
    ...
    def on_name_select(self, event):
        ...
        results = db_backend.search_purchasers_by_name(selected_name)
        if results:
    ...
    def on_phone_select(self, event):
        ...
        results = db_backend.search_purchasers_by_phone(selected_phone)
        if results:
    ...
    def on_name_focus(self, event):
        ...
        results = db_backend.search_purchasers_by_name('')
        names = [r[0] for r in results]
        self.name_combo['values'] = names
    ...
    def on_phone_focus(self, event):
       # Just update the list, don't force dropdown
        results = db_backend.search_purchasers_by_phone('')
        phones = [r[0] for r in results]
        self.phone_combo['values'] = phones
    ...
    def show_notification(self, message, title="Notification", on_close=None):
        ...
        # Create the label with wrapping
        label = tk.Label(
            popup,
            text=message,
            bg=colors["bg"],
            fg=colors["fg"],
            font=("Arial", 12),
            wraplength=380,  # Set wraplength a bit less than max width
            justify="left"
        )
        ...
        # Let popup size itself to content, then center
    ...
    def submit_form(self):
        ...
        if db_backend.check_purchaser_name_phone_match(name, phone):
            ...
            purchaser_id = db_backend.add_purchaser(name, place, phone, total_amount, amount_to_pay)
            if not purchaser_id:
                self.show_notification("Failed to add purchaser. Please try again.", title="Error")
                return
        ...
        for row in self.rows:
            ...
            db_backend.add_purchase_product(
            purchaser_id,
            item,
            qty,
            price,
        ...
        if amount_to_pay > 0:
            ...
            db_backend.add_purchase_payment(
                purchaser_id,
                payment_id,
        ...
        self.show_notification(confirm_msg, title="Payment Confirmation", on_close=self.reset_form)
...
class ProductAutocompleteEntry(tk.Entry):
    ...
    def check_autocomplete(self, event):
        ...
        if not typed:
            self.hide_listbox()
            return
        matches = db_backend.search_products_by_prefix(typed)  # Returns list of (item, price, desc)
        if matches:
            self.show_listbox([m[0] for m in matches], matches)
        else:
            self.hide_listbox()

    def show_listbox(self, names, matches):
        if not self.listbox:
            # Use the root window as parent for the Listbox for correct placement
            ...
            self.listbox.bind("<<ListboxSelect>>", lambda e: self.select_from_listbox(matches))
    ...
    def select_from_listbox(self, matches):
        ...
        # Autofill product name, price, and description
...
if __name__ == "__main__":
    ...
    frame = InwardAddFrame(root, get_colors)
