...
class InwardRecordPaymentFrame(tk.Frame):
    ...
    def on_record_payment(self):
        ...
        purchaser_id = db_backend.get_purchaser_id(self.current_name, self.current_phone, self.current_place)
        if not purchaser_id:
            self.show_themed_error("Not Found", "No matching purchase record found.")
            return

        # Add payment in backend
        success, new_remaining, new_status = db_backend.add_purchase_payment_to_record(purchaser_id, amt)
        if not success:
